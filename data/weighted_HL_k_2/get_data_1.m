...
u_vec_r = @(r,z) 3.*r.*(r-1); % 3.*r.^2 - 3.*r
u_vec_th = @(r,z) -3.*r.^2 + 2.*r;
u_vec_z = @(r,z) 0;
s_vec_r = @(r,z) 0;
s_vec_th = @(r,z) 0;
s_vec_z = @(r,z) (r.^2).*(r-1); % r.^3 - r.^2
f_vec_r = @(r,z) 3.*n.^2 - 3.*(n.^2)./r + 6.*n - 4.*n./r - 9;
f_vec_th = @(r,z) -3.*n.^2 + 2.*(n.^2)./r - 6.*n + 6.*n./r + 9;
f_vec_z = @(r,z) 0;
end